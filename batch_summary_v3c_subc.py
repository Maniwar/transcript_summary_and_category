...
@st.cache_data
def preprocess_comments_and_summarize(feedback_data, comment_column, max_length=60, min_length=10, max_tokens=1000, min_word_count=70):
    ...
    for chunk in short_chunks:
        token_count_before_batch = sum([get_token_count(comment, summarization_pipeline.tokenizer) for comment in chunk])
        print(f"Token count of batch before summarization: {token_count_before_batch}")
        summaries = summarization_pipeline(chunk, max_length=max_length, min_length=min_length, do_sample=False)
        token_count_after_batch = sum([get_token_count(summary['summary_text'], summarization_pipeline.tokenizer) for summary in summaries])
        print(f"Token count of batch after summarization: {token_count_after_batch}")
    ...
    for comment, tokens in long_comments:
        ...
        print(f"Token count of original comment: {tokens}")
        ...
        while get_token_count(full_summary, summarization_pipeline.tokenizer) > max_length:
            ...
            full_summary = " ".join(summary_parts)
            print(f"Re-summarized token count: {get_token_count(full_summary, summarization_pipeline.tokenizer)}")
