...
def cluster_emerging_issues_bertopic(trends_data, min_topic_size=5):
    ...
    - Clusters 'No Match' comments using BERTopic.
    - For each cluster, we gather the top 5 comments, summarize them individually,
      then unify them into a ~10-token label for the Sub-Category.
    - Also uses the top 3 words from the topic as the Keyphrase.
    - Noise points remain 'No Match'.
    """
    ...
    # Load summarization model for generating short labels
    model_summ, tokenizer_summ, device = get_summarization_model_and_tokenizer()

    # We want to find up to 5 top docs for each topic.
    # If get_representative_docs is empty, we can fallback.

    for i, topic in enumerate(topics):
        if topic == -1:  # Noise points
            ...
            # We'll do the label building once per topic.
            # We can also do a pre-pass building a dict topic->docs.
            # But for simplicity, we'll do it inline.
            # gather the top words for the keyphrase
            top_words = topic_model.get_topic(topic)
            keyphrase = ', '.join([word for word, _ in top_words[:3]])

            # gather representative docs
            docs_for_topic = topic_model.get_representative_docs(topic)
            if not docs_for_topic:
                # fallback: let's just pick the top 5 from all docs that have this topic
                # i is the index in the array, but we need all docs that match 'topic'.
                # let's do a separate pass after we build them.
                pass
            ...
            # We'll store a placeholder, then do a second pass.

    # Let's build an index of docs for each topic.
    from collections import defaultdict
    ...
    # Now for each topic, build one label from top 5 docs.
    for t, idx_list in topic_to_indices.items():
        # let's get up to 5
        limited = idx_list[:5]
        # gather those docs
        chosen_docs = [no_match_texts[i] for i in limited]
        # build short label
        ...
        # assign to all rows of that topic
        for i_ in idx_list:
